...
const viewerUrl = 'https://streetview.vn/';
...
// Load all data for the specified type from Streetview vector tiles
...
export default {
    ...
    getUserIds: async function(usernames) {
        ...
        // in streetview, a username can have multiple ids, when the same name is
    ...
    setStyles: function(context, hovered) {
        ...
        const markers = context.container().selectAll('.layer-streetview .viewfield-group');
        const sequences = context.container().selectAll('.layer-streetview .sequence');
        ...
        context.container().selectAll('.layer-streetview .viewfield-group .viewfield')
    ...
    updateUrlImage: function(imageKey) {
        if (!window.mocha) {
            ...
            if (imageKey) {
                hash.photo = 'streetview/' + imageKey;
            } else {
                delete hash.photo;
            }
    ...
    selectImage: function (context, id) {
        ...
        const viewerLink = `${viewerUrl}?picId=${d.id}`;
        ...
        let wrap = context.container()
            .select('.photoviewer .streetview-wrapper');
        ...
        const hdDomId = utilUniqueDomId('streetview-hd');

        let label = line1
            .append('label')
            .attr('for', hdDomId)
            .attr('class', 'streetview-hd');
        ...
        attribution
            ...
            .attr('href', 'mailto:support@streetview.vn')
            .call(t.append('streetview.report'));
    ...
    ensureViewerLoaded: function(context) {
        let imgWrap = context.container()
            .select('#ideditor-viewer-streetview-simple > img');
        ...
        let wrap = context.container()
            .select('.photoviewer')
            .selectAll('.streetview-wrapper')
            .data([0]);

        let wrapEnter = wrap.enter()
            .append('div')
            .attr('class', 'photo-wrapper streetview-wrapper')
            .classed('hide', true)
    ...
    showViewer: function (context) {
        let wrap = context.container().select('.photoviewer')
            .classed('hide', false);
        let isHidden = wrap.selectAll('.photo-wrapper.streetview-wrapper.hide').size();
        if (isHidden) {
            wrap
                .selectAll('.photo-wrapper:not(.streetview-wrapper)')
                .classed('hide', true);
            wrap
                .selectAll('.photo-wrapper.streetview-wrapper')
                .classed('hide', false);
        }
        return this;
    },
