...
export function svgStreetviewImages(projection, context, dispatch) {
    ...
    function init() {
        if (svgStreetviewImages.initialized) return;
        svgStreetviewImages.enabled = false;
        svgStreetviewImages.initialized = true;
    }


    function getService() {
        if (services.streetview && !_streetview) {
            _streetview = services.streetview;
            _streetview.event
                .on('viewerChanged', viewerChanged)
                .on('loadedLines', throttledRedraw)
                .on('loadedImages', throttledRedraw);
        } else if (!services.streetview && _streetview) {
            _streetview = null;
        }
    ...
    function drawImages(selection) {
        ...
        const enabled = svgStreetviewImages.enabled;
        const service = getService();

        layer = selection.selectAll('.layer-streetview')
            .data(service ? [0] : []);
    ...
    drawImages.enabled = function(_) {
        if (!arguments.length) return svgStreetviewImages.enabled;
        svgStreetviewImages.enabled = _;
        if (svgStreetviewImages.enabled) {
            showLayer();
            context.photos().on('change.streetview_images', update);
        } else {
            hideLayer();
            context.photos().on('change.streetview_images', null);
        }
